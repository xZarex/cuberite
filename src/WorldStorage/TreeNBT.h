...
using cCompound = std::map<AString, std::shared_ptr<cTag>>;
...
// {
// 	static_assert(std::is_same<typename std::decay<T>::type, T>::value, "Getter
// should not be cv-qualified"); 	static T & Get(TagStorage & a_TagStorage)
// 	{
...
// 	{
// 		return (m_TagId == TagId) ? &m_Payload.As<Detail::TypeFromTagId<TagId>>()
// : nullptr;
// 	}
...
class cList
{
	using cTagArray = std::vector<std::shared_ptr<cTag>>;
  ...
  public:
	...
	std::pair<iterator, bool> TryInsert(iterator a_Pos, std::shared_ptr<cTag> a_Tag)
	{
		if (!IsTagCompatible(a_Tag))
		{
			return {a_Pos, false};
		}

		return {m_Tags.insert(MutableIterator(a_Pos), a_Tag), true};
	}
	...
	Returns false if the tag is of the wrong type and cannot be inserted. */
	bool TryPushBack(const std::shared_ptr<cTag> a_Tag)
	{
		if (!IsTagCompatible(a_Tag))
		{
			return false;
	...
	template <typename Func> void Visit(iterator a_Pos, Func && a_Visitor)
	{
		std::visit(std::forward<Func>(a_Visitor), *MutableIterator(a_Pos));
	}
	...
	const std::shared_ptr<cTag> operator[](size_type a_Idx) const { return m_Tags[a_Idx]; }
	...
	void swap(cList & a_Other) noexcept { m_Tags.swap(a_Other.m_Tags); }
  ...
  private:
	...
	bool IsTagCompatible(const std::shared_ptr<cTag> a_Tag) const;
...
class cTag : public std::variant<
				 cEmptyTag, Int8, Int16, Int32, Int64, float, double, AString,
				 cCompound, cList, cByteArray, cIntArray>
{
	public:
	void assign(std::variant<
				 cEmptyTag, Int8, Int16, Int32, Int64, float, double, AString,
				 cCompound, cList, cByteArray, cIntArray> a)
	{
		std::variant<
				 cEmptyTag, Int8, Int16, Int32, Int64, float, double, AString,
				 cCompound, cList, cByteArray, cIntArray>::operator=(a);
	}
};
