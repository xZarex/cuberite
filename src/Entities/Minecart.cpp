...
void cMinecart::HandlePhysics(std::chrono::milliseconds a_Dt, cChunk & a_Chunk)
{
	...
	auto relPos = cChunkDef::AbsoluteToRelative(Vector3i(POSX_TOINT, POSY_TOINT, POSZ_TOINT));
...
void cMinecart::HandlePoweredRailPhysics(NIBBLETYPE a_RailMeta)
{
	...
	switch (a_RailMeta & 0x07)
	{
		case E_META_RAIL_ZM_ZP:  // NORTHSOUTH
		{
			...
			bool BlckCol = TestBlockCollision(a_RailMeta & 0x07), EntCol = TestEntityCollision(a_RailMeta  & 0x07);
			if (EntCol || BlckCol)
			{
				return;
			}
